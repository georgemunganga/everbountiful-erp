...
function quantity_calculate(item) {
  ...
  console.log("quantityItem", quantity);
...
// function quantity_calculate(item) {
//   var quantity = parseFloat($("#total_qntt_" + item).val()) || 0;
//   var available_quantity = parseFloat($(".available_quantity_" + item).val()) || 0;
//   var price_item = parseFloat($("#price_item_" + item).val()) || 0;
//   var invoice_discount = parseFloat($("#invoice_discount").val()) || 0;
//   var discount = parseFloat($("#discount_" + item).val()) || 0;
//   var vat_percent = parseFloat($("#vat_percent_" + item).val()) || 0;
//   var total_tax = parseFloat($("#total_tax_" + item).val()) || 0;
//   var total_discount = parseFloat($("#total_discount_" + item).val()) || 0;
//   var taxnumber = parseInt($("#txfieldnum").val()) || 0;
//   var dis_type = parseInt($("#discount_type").val()) || 0;
...
//   if (available_quantity > 0 && quantity > available_quantity) {
//     toastr["error"]("You can Sale maximum " + available_quantity + " Items");
//     $("#total_qntt_" + item).val(available_quantity);
//     quantity = available_quantity; // Update value to safe limit
//   }

//   // Main price and discount logic
//   if (quantity > 0 || discount > 0 || vat_percent > 0) {
//     var price = quantity * price_item;
//     var dis = 0;
//     var temp = 0;
...
//     for (var i = 0; i < taxnumber; i++) {
//       var taxRate = parseFloat($("#total_tax" + i + "_" + item).val()) || 0;
//       var tax = temp * taxRate;
//       $("#all_tax" + i + "_" + item).val(tax);
//     }
//   } else {
//     var n = quantity * price_item;
...
function updateProductModal(productname, stock, model, unit, price, productId) 
{
    // $("#updated_product").modal("show");
    currentEditingProductId = productId;
    var totalPriceElement = document.getElementById("total_price_" + productId);
    var quantity = document.getElementById("total_qntt_" + productId);
    var discount = document.getElementById("discount_" + productId);
    var vat = document.getElementById("vat_percent_" + productId);
    var vat_value = document.getElementById("vat_value_" + productId);
    var discount_value = document.getElementById("discount_value_" + productId);
    if (totalPriceElement) 
      {
      console.log("totalPrice:", totalPriceElement.value);
    } else 
    {
      console.warn("Element with ID total_price_" + productId + " not found!");
    }
    document.getElementById("pmodel_rate").value = price;
    document.getElementById("pmodel_item_qty").value = quantity.value;
    document.getElementById("total_price_").innerHTML = totalPriceElement.value;
    var base_url = document.getElementById("baseurl").value;
    document.getElementById("pmodel_product_name").innerHTML = productname;
    document.getElementById("pmodel_quantity").value = quantity.value;
    document.getElementById("pmodel_discount").value = discount.value;
    document.getElementById("pmodel_discount_value").value = discount_value.value;
    document.getElementById("pmodel_vat").value = vat.value;
    document.getElementById("pmodel_vat_value").value = vat_value.value;
...
function applyProductUpdate() 
{
  ...
  const qty = document.getElementById("pmodel_quantity").value || 0;
  const rate = document.getElementById("pmodel_rate").value || 0;
  const discount = document.getElementById("pmodel_discount").value || 0;
  const discountValue = document.getElementById("pmodel_discount_value").value || 0;
  const vat = document.getElementById("pmodel_vat").value || 0;
  const vatValue = document.getElementById("pmodel_vat_value").value || 0;
  // const invoiceDiscount = document.getElementById("invoice_discount_percent").value || 0;
  
  // Update values in table row fields
  let qtyInput = document.getElementById("total_qntt_" + productId);
  if (qtyInput) {
      qtyInput.value = qty;
  } else {
      console.warn("Element not found: total_qntt_" + productId);
  }
  
  let priceInput = document.getElementById("price_item_" + productId);
if (priceInput) {
    priceInput.value = rate;
} else {
    console.warn("Price input not found for productId:", productId);
}
...
if (discountInput) {
    ...
    console.warn("Discount input not found for productId:", productId);
}

let discountValueInput = document.getElementById("discount_value_" + productId);
if (discountValueInput) {
    discountValueInput.value = discountValue;
} else {
    console.warn("Discount value input not found for productId:", productId);
}

let vatPercentInput = document.getElementById("vat_percent_" + productId);
if (vatPercentInput) {
    vatPercentInput.value = vat;
} else {
    console.warn("VAT percent input not found for productId:", productId);
}

let vatValueInput = document.getElementById("vat_value_" + productId);
if (vatValueInput) {
    vatValueInput.value = vatValue;
} else {
    console.warn("VAT value input not found for productId:", productId);
}

  // document.getElementById("total_discount_amount").innerHTML = invoiceDiscount;
  // Calculate new total price
  const total = (qty * rate) - discountValue + parseFloat(vatValue);
  console.log("total", total);

  let totalPriceInput = document.getElementById("total_price_" + productId);
  if (totalPriceInput) {
      totalPriceInput.value = total.toFixed(2);
  } else {
      console.warn("Total price input not found for productId:", productId);
  }
  
  quantity_calculate(productId)

  // Optional: close modal if needed
  $('#updated_product').modal('hide');
  $('#discount_modal').modal('hide');
  $('#shippint_cost_modal').modal('hide');
}
...
function deleteRow(t, nid) {
  ...
  if (1 == a) alert("There only one row you can't delete.");
    ...
    // Remove product hidden ID when row is deleted
    $("#SchoolHiddenId_" + nid).val('');
    calculateSum();
    invoice_paidamount();
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.status == true) {
          ...
          $(".active_qty").text("");
          $('#multipay_modal').modal('hide');
          getDraftList();

      
          // Check if the sale is a draft entry, and show the confirmation only if it's not
          if (!data.invoice_status) {
            ...
            // For draft sales, just reset the form without showing the confirmation
            $("#addinvoice tbody tr").remove();
            $("#gui_sale_insert").trigger("reset");
            $("#n_total").val("");
            $("#net_total_text").text("0.00");
...
function calculateTotalQuantity() {
  ...
  $("input[name='product_quantity[]']").each(function () {
      var qty = parseFloat($(this).val()) ; // ✅ Corrected
       if (!isNaN(qty)) {
          totalQty += qty;
      }
  });
  $("#total_quantity").text(totalQty); // ✅ Also use .val() here
}
...
function invoice_no_check() {
  ...
  $.ajax({
    ...
    method: "GET", // Use 'GET' as the HTTP method
