...
class Quotation extends MX_Controller
{
            ...
            if ($this->form_validation->run() === true) {
                ...
                if ($smultipaytype && $smultipayamount) {
                    ...
                    for ($i = 0; $i < count($smultipaytype); $i++) {
                        ...
                        $COAIDserv = $smultipaytype[$i] ?? 0;
                        $amount_payserv = $smultipayamount[$i] ?? 0;
    ...
    public function invoice_pdf_generate($invoice_id = null)
    {
        ...
        $totalbal = $invoice_detail[0]['total_amount'] ?? 0 + $invoice_detail[0]['prevous_due'] ?? 0;
    ...
    public function quotation_download($quot_id = null)
    {
        ...
        // Load the PDF generator library
        $this->load->library('pdfgenerator');

        // Generate the HTML content for the PDF
        echo $page = $this->load->view('quotation/quotation_download', $data, true);

        exit;

        // Generate PDF output
        $pdfContent = $this->pdfgenerator->generate($page, 'quotation_invoice', false);  // FALSE returns PDF data instead of streaming

        // Define the file name
        $file_name = "quotation_" . time() . ".pdf";
        $file_path = "assets/data/pdf/quotation/$file_name";
        ...
        // Optionally, delete the file after downloading
        unlink($file_path);
