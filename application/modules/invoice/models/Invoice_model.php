...
class Invoice_model extends CI_Model
{
    ...
    public function get_sales_summary($from_date = null, $to_date = null)
{

    $this->db->select('DATE(i.date) as sale_date, COUNT(DISTINCT i.id) as total_sales, 
    ...
    public function get_invoice_id_draft()
    {
    ...
    {
        return $this->db->get_where('invoice', ['invoice_id' => $invoice_id])->row_array();
    }

    public function get_invoice_details($invoice_id)
    {
        $invoice_sl = $this->db->select('id')
            ->from('invoice')
        ...
        if ($invoice_sl) {
            $this->db->select('i.*, p.product_name, p.product_model, p.price, p.unit, p.product_vat, p.image');
            $this->db->from('invoice_details i');
            $this->db->join('product_information p', 'i.product_id = p.product_id', 'left');
            $this->db->where('i.invoice_id', $invoice_sl->id);
    ...
    public function invoice_entry($incremented_id)
    {
        ...
        $is_credit_sale = $this->input->post('credit_sale') ? true : false;
        ...
        if ($draft_invoice) {
            // যদি draft_invoice থাকে, তাহলে update হবে
            $this->db->where('invoice_id', $invoice_no);
            $this->db->update('invoice', $datainv);

            // এখানে এখন primary id খুঁজে বের করতে হবে
            $this->db->select('id');
            ...
            if ($invoice_data) {
                $inv_insert_id = $invoice_data->id; // এখানেই আসল primary key id
            } else {
                $inv_insert_id = null; // যদি না পাওয়া যায়
            }
        } else {
            // না থাকলে নতুন করে insert হবে
            $this->db->insert('invoice', $datainv);
            $inv_insert_id = $this->db->insert_id();
        }
        ...
        for ($i = 0, $n = count($p_id); $i < $n; $i++) {
            ...
            if (!empty($quantity)) {
                ...
                if ($draft_invoice) {
                    // Draft থাকলে check করবো invoice_id + product_id মিলেছে কিনা
                    $this->db->where('invoice_id', $inv_insert_id);
                    $this->db->where('product_id', $product_id);
                    $query = $this->db->get('invoice_details');
                    if ($query->num_rows() > 0) {
                        // আগে থেকেই থাকলে Update করবো
                        $this->db->where('invoice_id', $inv_insert_id);
                        $this->db->where('product_id', $product_id);
                        $this->db->update('invoice_details', $data1);
                    } else {
                        // না থাকলে Insert করবো
                        $data1['invoice_details_id'] = $this->generator(15);
                        $this->db->insert('invoice_details', $data1);
                    }
                } else {
                    // Draft না থাকলে সরাসরি Insert করবো
                    $data1['invoice_details_id'] = $this->generator(15);
                    $this->db->insert('invoice_details', $data1);
                }

                // যেকোনো অবস্থায় Product Price update হবে
    ...
    public function bdtask_invoice_pos_print_direct($invoice_id = null)
    {
        ...
        $totalbal = (!empty($invoice_detail) && is_array($invoice_detail)) 
        ? $invoice_detail[0]['total_amount'] + $invoice_detail[0]['prevous_due'] 
        : 0;
    
        $user_id = (!empty($invoice_detail) && is_array($invoice_detail) && isset($invoice_detail[0]['sales_by']))
        ? $invoice_detail[0]['sales_by']
        : null;
