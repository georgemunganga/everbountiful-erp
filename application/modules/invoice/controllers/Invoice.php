...
class Invoice extends MX_Controller
{
    ...
    public function test_ai_summary()
    {

        $data = $this->invoice_model->get_sales_summary('2024-06-01', '2025-06-15');
        $summary_text = "Sales summary from 2024-06-01 to 2025-06-15:\n";

        foreach ($data as $row) {
            $summary_text .= "Date: {$row->sale_date}, Total Sales: {$row->total_sales}, Total Quantity: {$row->total_qty}, Total Amount: {$row->total_sales_amount} BDT\n";
        }

        $prompt = "You are a smart business analyst. Please analyze the following sales summary and give insights, trends, or recommendations:\n\n" . $summary_text;
    ...
    public function bdtask_manual_sales_insert()
    {
        ...
        if (!empty($cash_amount) || $cash_amount != '' && !empty($card_amount) || $card_amount != '') {
            $total_amount = $cash_amount + $card_amount;
            $grand_total_amount = $this->input->post('grand_total_price', TRUE);
            // Amount Condition Checking..
            if ($grand_total_amount != $total_amount) {
                $data['status'] = false;
                $data['exception'] = 'Please give full pay payment equal to grand Total amount';
                echo json_encode($data);
                return;
            }
        }
    ...
    public function bdtask_gui_pos()
    {
        ...
        echo modules::run('template/posLayout', $data);
    }
    ...
    public function gui_pos_invoice()
    {
        ...
        if (!empty($product_details)) {
            ...
            if (empty($pur_product_batch)) {
                ...
                $html .= "<select name=\"serial_no[]\"   class=\"serial_no_1 form-control batch_no_td \" required onchange=\"invoice_product_batch('" . $product_details->product_id . "')\" id=\"serial_no_" . $product_details->product_id . "\">";
            ...
            $tr .= "<tr id=\"row_" . $product_details->product_id . "\">
                            <td class=\"text-center\">
                                <button type=\"button\" data-toggle=\"modal\" onclick=\"updateProductModal('" . $product_details->product_name . "','" . $product_details->total_product . "','" . $product_details->product_vat . "','" . $product_details->unit . "','" . $product_details->price . "', '" . $product_details->product_id . "')\" data-target=\"#updated_product\" 
                                    class=\"btn_edit text-center\"><i class=\"fa fa-pencil-square-o\"
                                        aria-hidden=\"true\"></i></button>
                            </td>
                        <td>
                            <input type=\"text\" name=\"product_name\" onkeypress=\"invoice_productList('" . $product_details->product_id . "');\" class=\"form-control product_name_td productSelection \" value='" . $product_details->product_name . "- (" . $product_details->product_model . ")" . "' placeholder='" . display('product_name') . "' required=\"\"  tabindex=\"\" readonly>
                        ...
                        <td>" . $html . "</td>
                        <td>
                            <input type=\"text\" name=\"available_quantity[]\" class=\"form-control available_quantity_td text-right available_quantity_" . $product_details->product_id . "\" value='' readonly=\"\" id=\"available_quantity_" . $product_details->product_id . "\"/>
                        </td>
                        <td>
                            <input type=\"text\" name=\"product_quantity[]\" onkeyup=\"quantity_calculate('" . $product_details->product_id . "');\" onchange=\"quantity_calculate('" . $product_details->product_id . "');\" class=\"total_qntt_" . $product_details->product_id . " form-control qty_td text-right\" id=\"total_qntt_" . $product_details->product_id . "\" placeholder=\"0.00\" min=\"0\" value='" . $qty . "' required=\"required\"/>
                        </td>
                        <td style=\"width:85px\">
                            <input type=\"text\" name=\"product_rate[]\" onkeyup=\"quantity_calculate('" . $product_details->product_id . "');\" onchange=\"quantity_calculate('" . $product_details->product_id . "');\" value='" . $product_details->price . "' id=\"price_item_" . $product_details->product_id . "\" class=\"price_item1 form-control price_td text-right\" required placeholder=\"0.00\" min=\"0\"/>
                        </td>
                        ...
                        <input class=\"total_price form-control total_price_td text-right\" type=\"text\" name=\"total_price[]\" id=\"total_price_" . $product_details->product_id . "\" value='0.00' tabindex=\"-1\" readonly=\"readonly\"/>
                        </td>
                        <td>";
            $sl = 0;
    ...
    {
        $invoice_id_list = $this->invoice_model->get_invoice_id_draft();

        // foreach ($invoice_id_list as $value) {
        //     $json_invoice_id[] = array('value' => $value['invoice_id']);
        // }
        echo json_encode($invoice_id_list);
    }
    ...
    public function get_invoice_data()
    {
        ...
        $details = $this->Invoice_model->get_invoice_details($invoice_id);
