...
class Report_model extends CI_Model
{
    ...
    // Get Table Report Summary for AI Module

    public function get_table_summary($date_from, $date_to)
    {
        $result = [];

        // ✅ Sales (Invoice)
        $sales = $this->db->select('date as date, "Sales" as type, total_amount as amount')
            ->from('invoice')
            ->where('date >=', $date_from)
            ->where('date <=', $date_to)
            ->where('status', 1)
            ->get()->result_array();
        $result = array_merge($result, $sales);

        // ✅ Purchase
        $purchase = $this->db->select('purchase_date as date, "Purchase" as type, grand_total_amount as amount')
            ->from('product_purchase')
            ->where('purchase_date >=', $date_from)
            ->where('purchase_date <=', $date_to)
            ->where('status', 1)
            ->get()->result_array();
        $result = array_merge($result, $purchase);

        // ✅ Sales Return (product_return with invoice_id)
        $sales_return = $this->db->select('date_return as date, "Sales Return" as type, net_total_amount as amount')
            ->from('product_return')
            ->where('invoice_id IS NOT NULL')
            ->where('date_return >=', $date_from)
            ->where('date_return <=', $date_to)
            ->get()->result_array();
        $result = array_merge($result, $sales_return);

        // ✅ Supplier Return (product_return with purchase_id)
        $supplier_return = $this->db->select('date_return as date, "Supplier Return" as type, net_total_amount as amount')
            ->from('product_return')
            ->where('purchase_id IS NOT NULL')
            ->where('date_return >=', $date_from)
            ->where('date_return <=', $date_to)
            ->get()->result_array();
        $result = array_merge($result, $supplier_return);

        // ✅ Stock (Product Information)
        $stock_data = $this->db->select('product_id, product_name, product_model, price')
